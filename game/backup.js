...
function changeLabel() {
    label.textContent = `Number of Players: ${selector.value}`
}
...
class Enemy {
    constructor(){
        ...
        let dSpawnx = this.x - activeLevel.spawn1.x
        let dSpawny = this.y - activeLevel.spawn1.x
        ...
        this.ySpeed = 0
        while (this.ySpeed === 0) {
            this.ySpeed = Math.floor(Math.random() * (6 - 3 + 1)) + 3
        }
    ...
    update(){
        ...
        // enemies used to disapear if target was in a corner because x and y got set no NaN
        if (this.x + dx >= 0 && this.x + dx + this.w <= canvas.width &&
            this.y + dy >= 0 && this.y + dy + this.h <= canvas.height) {
            // check if new pos is more than target pos. to stop jittering
            if (this.middlePos.x + dx > this.targetPos.x ||
                this.middlePos.y + dy > this.targetPos.y ||
                this.middlePos.x - dx < this.targetPos.x ||
                this.middlePos.y - dy < this.targetPos.y) 
                {
                this.x += dx;
                this.y += dy;
